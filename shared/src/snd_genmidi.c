...
/* OPL sustain attenuation is 3 dB/step, with level 15 effectively off. */
static const int16_t gm_sustain_gain[16] = {
    256, 181, 128, 91, 64, 46, 32, 23, 16, 11, 8, 6, 4, 3, 2, 0};

/* The generic FM backend stores envelope times, while OPL stores 4-bit rates.
 * This intentionally approximates the rate curve instead of claiming chip
 * timing accuracy.  Rate zero is represented as a very slow segment. */
static const uint16_t gm_rate_ms[16] = {
    60000u, 12000u, 8000u, 5000u, 3000u, 1800u, 1000u, 600u,
    350u, 200u, 120u, 70u, 40u, 20u, 8u, 2u};

static const uint8_t gm_wave_map[8] = {
    SND_WAVE_SINE, SND_WAVE_SINE, SND_WAVE_TRIANGLE, SND_WAVE_SQUARE,
    SND_WAVE_SINE, SND_WAVE_TRIANGLE, SND_WAVE_SQUARE, SND_WAVE_SAW};
...
static void gm_convert_operator(const uint8_t SND_PTR *raw,
  ...
  out->wave = gm_wave_map[raw[3] & 0x07u];
  out->env.attack_ms = gm_rate_ms[(attack >> 4) & 0x0fu];
  out->env.decay_ms = gm_rate_ms[attack & 0x0fu];
  out->env.release_ms = gm_rate_ms[sustain & 0x0fu];

  /* OPL's EGT/sustain bit decides whether the envelope holds at sustain.
   * A non-sustaining OPL voice continues decaying, which the generic ADSR can
   * most closely represent by decaying to zero while the key remains down. */
  if ((tremolo & 0x20u) != 0u)
    out->env.sustain = gm_sustain_gain[(sustain >> 4) & 0x0fu];
  else
    out->env.sustain = 0;
}
...
static void gm_convert_voice(const uint8_t SND_PTR *raw,
   ...
   * only has the OPL connection bit and 3-bit feedback field, so these are
   * intentionally coarse translations rather than fake register accuracy. */
   ...
   * operator, so carrier unity is the closest equivalent. */
...
static void gm_convert_instrument(const uint8_t SND_PTR *raw,
  ...
  out->layer_count = (uint8_t)(((flags & SND_GENMIDI_FLAG_2VOICE) != 0u) ? 2u
                                                                            : 1u);
