...
typedef struct mw_trace {
  FILE *fp;
  uint64_t sequence;
} mw_trace_t;

static void mw_trace_register(void *user, long frame,
                              uint16_t reg, uint8_t value) {
  mw_trace_t *trace = (mw_trace_t *)user;
  if (!trace || !trace->fp)
    return;
  fprintf(trace->fp, "%llu,%ld,0x%03X,0x%02X\n",
          (unsigned long long)trace->sequence++,
          frame, (unsigned int)reg, (unsigned int)value);
}
...
static void mw_usage(const char *exe) {
  ...
  printf("  --freq-split N DMX table split: 284 (Chocolate/DSDA default) or 283\n");
  printf("  --trace FILE  write ordered OPL register calls as CSV\n");
...
static int mw_parse_args(int argc, char **argv, mw_args_t *args) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (strcmp(arg, "--wad") && strcmp(arg, "--music") &&
        ...
        strcmp(arg, "--gain") && strcmp(arg, "--freq-split") &&
        strcmp(arg, "--trace") && strcmp(arg, "--accum")) {
    ...
    } else if (!strcmp(arg, "--freq-split")) {
      if (!mw_parse_int(value, 283, 284, &args->freq_split) ||
          (args->freq_split != 283 && args->freq_split != 284)) {
        fprintf(stderr, "ERROR: --freq-split must be 283 or 284.\n");
        return 0;
      }
...
int main(int argc, char **argv) {
  ...
  snd_genmidi_opl_set_freq_model(
      &opl,
      (args.freq_split == 283)
          ? SND_GENMIDI_OPL_FREQ_DMX_BUG_283
          : SND_GENMIDI_OPL_FREQ_DOOM19_284);

  if (args.trace_path) {
    trace.fp = fopen(args.trace_path, "wb");
    if (!trace.fp) {
      fprintf(stderr, "ERROR: cannot create trace: %s\n",
              args.trace_path);
      goto done;
    }
    fprintf(trace.fp, "sequence,frame,register,value\n");
    snd_genmidi_opl_set_trace(&opl, mw_trace_register, &trace);

    /* Re-run startup initialization with tracing enabled so the CSV contains
     * the complete driver register stream from reset onward. */
    snd_genmidi_opl_reset(&opl);
  }
  ...
  printf("Backend:                  Nuked OPL3, strict Doom/DMX OPL2\n");
  printf("DMX driver target:        Doom 1.9 / Ultimate Doom\n");
  printf("DMX frequency split:      %d (%s)\n",
         args.freq_split,
         args.freq_split == 284
             ? "Chocolate/DSDA/Woof"
             : "libADLMIDI reverse-engineered side-bug");
  ...
  printf("OPL voices stolen:        %lu\n",
         opl.voices_stolen);
  printf("Secondary voices dropped: %lu\n",
         opl.secondary_voices_dropped);
  printf("OPL register writes:      %lu\n",
         opl.register_writes);
